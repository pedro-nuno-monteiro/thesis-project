...
def plot_subcarrier_amplitude_vs_sample(
	magnitude_data: csi_map,
	scenario: str,
	user: str,
	activity: str,
	...
	alpha: float = 0.6,
	linewidth: float = 1.1,
	max_subcarriers: int | None = None,
	show_legend: bool = True,
	legend_max_entries: int = 20,
	save_path: str | Path | None = None,
) -> None:
	"""Plot amplitude (y) vs sample index (x), using one curve per subcarrier."""
	try:
		magnitude = magnitude_data[scenario][user][activity][esp][trial]
	except KeyError:
		print(
	...
	n_samples, n_subcarriers = magnitude.shape
	sample_idx = np.arange(n_samples)

	if max_subcarriers is not None and max_subcarriers > 0 and n_subcarriers > max_subcarriers:
		subcarrier_indices = np.linspace(0, n_subcarriers - 1, max_subcarriers, dtype=int)
		subcarrier_indices = np.unique(subcarrier_indices)
		print(
			f"Plotting {len(subcarrier_indices)} subcarriers "
			f"(downsampled from {n_subcarriers}).",
		)
	else:
		subcarrier_indices = np.arange(n_subcarriers)
		print(f"Plotting all {n_subcarriers} subcarriers.")

	if save_path is None:
		save_path = (
			Path("images")
			/ "saved_graphs"
			/ "2D graph amplitude vs sample"
			/ _measurement_plot_filename(
				scenario,
				user,
				activity,
				esp,
				trial,
				suffix="all_subcarriers_vs_sample",
			)
		)
	...
	colors = cmap(np.linspace(0.05, 0.95, len(subcarrier_indices)))
	line_handles = []

	for color, subcarrier_idx in zip(colors, subcarrier_indices):
		(line,) = ax.plot(
			sample_idx,
			magnitude[:, subcarrier_idx],
			color=color,
			alpha=alpha,
			linewidth=linewidth,
			label=f"SC {subcarrier_idx}",
		)
		line_handles.append(line)

	# Overlay mean amplitude profile across subcarriers for visual reference.
	mean_profile = np.mean(magnitude[:, subcarrier_indices], axis=1)
	(mean_handle,) = ax.plot(
		sample_idx,
		mean_profile,
		color="black",
	...
	ax.set_title(
		f"{scenario_id_to_label(scenario)} | {user} | {activity} | {esp} | {trial}\n"
		"Amplitude vs Sample (one curve per subcarrier)",
	)
	...
	if show_legend:
		if len(line_handles) <= legend_max_entries:
			handles = [*line_handles, mean_handle]
		else:
			picked_idx = np.linspace(0, len(line_handles) - 1, legend_max_entries, dtype=int)
			handles = [line_handles[idx] for idx in np.unique(picked_idx)]
			handles.append(mean_handle)
			print(
				"Legend shortened to "
				f"{len(handles) - 1} subcarriers (out of {len(line_handles)}) plus Mean.",
			)
