...
def transform_name(name, existing_counts):
...
def process(src: Path, dst: Path, dry_run=False, recursive=True):
    count = 0
    existing_counts = {}
    files = list(src.rglob("*.csv")) if recursive else list(src.glob("*.csv"))
