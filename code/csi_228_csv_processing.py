...
PROCESSOR_VERSION = "five-ghz-228-v1"
...
def _process_228_cached(
    ...
    return _process_cached(job, _process_228_file, PROCESSOR_VERSION, options)
        ...
        for location_key, users_map in locations_map.items():
            for user_key, esps_map in users_map.items():
                for esp_key, trials_map in esps_map.items():
                    ...
                    if not FIVE_GHZ_MIN_ESP_ID <= esp_id <= FIVE_GHZ_MAX_ESP_ID:
                        continue
...
def process_csv_files(  # noqa: PLR0913
    ...
    *,
    ...
    results, stats = _run_jobs(jobs, _process_228_cached, options)
